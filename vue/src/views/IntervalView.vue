...
export default {
    setup(){
        ...
        onMounted(async () => {
          await getTopAndOtherGroupSignals(topGroupSignals, otherGroupSignals, object, group, activeInterval)
    ...
    <div :style="{ 'padding': '0px 20px 0px 20px' }">
        <div class="container-fluid">
            <div class="row align-items-center">
                <div class="col-md-7 text-begin" :style="{ 'padding': '0px 10px 0px 0px' }">
                    <h4>{{ activeIntervalLabel }}</h4>
                </div>
                <div class="col-md-2 text-end">
                    <Button @click="onButtonPdfClick">PDF отчет</Button>
                </div>
                <div class="col-md-3" v-if="intervalElementsDisable" :style="{ 'padding': '0px 0px 0px 10px' }">
        ...
        <div class="container-fluid interval-padding-signals">
            ...
            <div class="row" v-for="top in topGroupSignals">
                <div class="col">
                    <Checkbox v-model="topCheckbox" :input-id="top.kks" name="topCheckobx" :value="top.kks"></Checkbox>
                    <label :for="top.kks">{{ top.kks + ' (' + top.description + ')' }}</label>
                </div>
            </div>
            ...
            <div class="row" v-for="other in otherGroupSignals">
                <div class="col">
                    <Checkbox v-model="otherCheckbox" :input-id="other.kks" name="otherCheckbox" :value="other.kks"></Checkbox>
                    <label :for="other.kks">{{ other.kks + ' (' + other.description + ')'}}</label></div>
            </div>
        </div>
        <br />
